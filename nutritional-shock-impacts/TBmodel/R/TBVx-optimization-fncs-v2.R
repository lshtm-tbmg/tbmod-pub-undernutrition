# Nov 14, 2022. Model version 3.2.8.9.6
fit.model = function(param.values=NULL, fitted.params=NULL, xmlparams=NULL, targets=NULL, logfn=NULL, mdl=NULL, pwr=NULL){
  cat(param.values,"\n")
  updated.fitted.params = modify.input.csv(fitted.params,param.values)
  xmlparams$xml$doc = update.parameters(xmlparams$xml$doc,updated.fitted.params)
  initialized.params = initialize.model.parameters(xmlparams)
  output = run.model(initialized.params)
  fit    = eval.output.vs.targets(output,targets,p=initialized.params)
...
create.logr = function(paths=NULL,params=NULL){
  ...
  logfilename   = create.filename(path=paths$log.dir,model.params=params,runtype="optim")
...
final.run.and.write.result = function(paths, params, constant.params, fitted.params, logfn=NULL, result=NULL, targets=NULL){
  
  new.fitted.params  = modify.input.csv(input.csv=fitted.params,new.parameter.values=result$par)
  params$xml$doc     = update.parameters(params$xml$doc,new.fitted.params)
  initialized.params = initialize.model.parameters(params)
  output             = run.model(initialized.params)
  out                = eval.output.vs.targets(output,targets,p=initialized.params)
  
  logfn(fit=out, pvalues=result$par)
  score = sum(out$fit)/length(out$fit)
  if (is.na(score)){
    logr(level="ERROR",msg="score NA !?")
  }else{
    fname=create.filename(paths$log.dir,model.params=initialized.params,runnr=runnr,runtype="optim",score=round(100*score,0))
...
#' Runs the model and tries to find a set of parameter values that minimizes the weighted sum of squares of differences between target values and model values
#' Results are written to files in the logs folder
...
#' @return NULL
#' @export
run.optimizer = function(paths=NULL, new.parameter.values=NULL, constant.new.parameter.values=F, lmcontrolcsv=NULL, nruns=1, pwr=NULL){
  ...
  targets             = read.targets(paths$targets)
  params              = read.model.parameters(paths)
  params$xml$doc      = update.parameters(params$xml$doc,constant.params)
  
  logr               = create.logr(paths,params)  
  ...
  if (!is.null(new.parameter.values)){
    ...
    result           = nls.lm(par             = new.params, 
                              ...
                              mdl             = model, 
                              pwr             = pwr)
    final.run.and.write.result(paths, params, constant.params, fitted.params, logr, result, targets)
  }else{
    for (i in 1:nruns){
