...
#' 
#' @param xmldoc the in memory XML document
#' @param new.params a data frame read from (usually) the 'input.csv' with parameter values to be replaced and to be located by the xpath in the .csv
#' 
...
#' assigns new values to attributes in an XML document using the xpath to locate the set of attributes
#' 
#' @param xmldoc the in memory XML document
#' @param xpath  xpath to the attributes
#' @param attrname attribute name
#' @param newvalues the new attribute values
#' 
#' @return the modified XML document
set.node.attrs = function(xmldoc, xpath, attrname=NULL, newvalues=NULL){
  ...
  for (i in seq_along(newvalues)){
    xml_set_attr(nodeset[i],attrname,newvalues[i])
  }
  xmldoc
}
