...
#' checks that target states are defined for dimensions 
#' 
#' @param  target.states  a named character vector of comma separated state name by dimension
#' @param  p              initialized model parameters
#' 
#' @return NULL
#' @export
validate.target.states=function(target.states=NULL,p=NULL){
  substrs = stri_split_fixed(target.states,",")
  for (i in seq_along(names(target.states))){
    check = substrs[[i]] %in% p$DIMNAMESLIST[[names(target.states)[i]]]
    assert_that(all(check),msg = paste("target states",paste(substrs[[i]][!check], collapse=","),"not found in",paste(p$DIMNAMESLIST[[names(target.states)[i]]],collapse=",")))
  }
}

#' generates a query for a target ; either the numerator or the denominator
#' 
#' @param  target  one target (i.e. either numerator or denominator in the targets file)
#' @param  p       initialized model parameters
#' 
#' @return a string with the generated query
#' @export
query.from.target=function(target=NA,p=NULL){ # NOTE the dead are excluded in the function calling this fn
  ...
  NAs = is.na(target)
  target[NAs]="" 
  validate.target.states(target[!NAs & mult],p)
...
#' reads a csv with defined targets, checks for consistency and produces a data frame
#' 
#' @param  fname  file name of the csv to read
#' @return a data frame with targets
#' @export
...
#' 
#' @param  z a data frame (either numerator or denominator targets) 
#' @return the modified data frame with non alive stocks removed from the columns VXa, SES, RISK, HIV, TB
#' @export
...
#' helper function called from calc.targets() to calculate a count value
#' 
#' @param  z a data frame with stocks output
#' @param  target a subset of the columns of a single row of the targets defining either the numerator or the denominator to be calculated
#' @param  p initialized model parameters
#' @return a single number i.e. the result of querying the stocks data frame with a query generated from the target parameter
#' @export
...
#' helper function called from calc.targets() to calculate a stock value (excluding non alive states)
#' 
#' @param  z a data frame with stocks output
#' @param  target a subset of the columns of a single row of the targets defining either the numerator or the denominator to be calculated
#' @param  p initialized model parameters
...
#' helper function called from calc.targets() to calculate a flow value 
#' 
#' @param  z a data frame with flows output
#' @param  target a subset of the columns of a single row of the targets defining either the numerator or the denominator to be calculated
#' @param  p initialized model parameters
#' @return a single number i.e. the result of querying the flows data frame with a query generated from the target parameter to produce either inflow or outflow
#' @export
calc.flow=function(z,target,p=NULL){
  causes = stri_trim_both(unlist(strsplit(target$cause,",")))
  allowed_causes = c('Tm',paste0(names(p$inci.dim.names[names(p$inci.dim.names)!=""]),'Xi'),
                          paste0(p$DIMNAMES,'p'),
                          paste0('TBtr_',names(p$TBtr)),
                          paste0('HIVtr_',names(p$HIVtr)),
                          paste0('RISKtr_',names(p$RISKtr))
                     )
...
#' calculates target model values from the output generated by running the model
#' 
#' @param  z the output from running the model: a list with elements stocks and flows
#' @param  targets a data frame with targets (from reading the targets csv)
#' @param  p initialized model parameters
#' @return the targets data frame with a new column 'model' with values calculated from the model output
#' @export
...
#' 
#' 
#' this function calls calc.targets() to do calculate non-combined targets and then calculates combined targets from the calc.targets() output
#' 
#' @param  output the output from running the model: a list with elements stocks and flows
#' @param  targets a data frame with targets (from reading the targets csv)
#' @param  p initialized model parameters
#' @return the targets data frame with new columns with values calculated from the model output
#' @export
