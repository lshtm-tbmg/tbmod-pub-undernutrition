...
### Sample usage: Rscript AutoEmulateRerun.R -c AFG -p input.csv -t target_em.csv -x XMLinput.xml -n 10 -s 0 -m 1000 -f 0.4
### Details of flags:
...
## -x XML file including .xml extension (Default XMLinput_target.xml)
...
print("renv details:")
print(renv::paths$root())
print(renv::paths$library())
print(renv::paths$cache())
...
run.model <- function(points, wave, res) {
  all_outputs <- apply(points, 1, function(x) model$run(model$paths, x, FALSE)$hits)
...
xfile <- if(is.null(opts$xml)) "XMLinput_target.xml" else opts$xml
